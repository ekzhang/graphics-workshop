...
class ValueBase {
    ...
    include() { }
    includeCallArguments(_context, _args) { }
    mayModifyThisWhenCalledAtPath() { return true; }
}
function includeAll(context, args) {
    for (const arg of args) {
        arg.include(context, false);
    }
}
const UNKNOWN_EXPRESSION = new class extends ValueBase {
    includeCallArguments(context, args) {
        includeAll(context, args);
    }
};
const UNDEFINED_EXPRESSION = new class extends ValueBase {
    getLiteralValueAtPath() {
        return undefined;
    }
};
...
class UnknownArrayExpression extends ValueBase {
    constructor() {
        super(...arguments);
        this.included = false;
    }
...
const UNKNOWN_LITERAL_BOOLEAN = new class extends ValueBase {
    getReturnExpressionWhenCalledAtPath(path) {
        if (path.length === 1) {
...
const UNKNOWN_LITERAL_NUMBER = new class extends ValueBase {
    getReturnExpressionWhenCalledAtPath(path) {
        if (path.length === 1) {
    ...
    hasEffectsWhenAccessedAtPath(path) { return path.length > 1; }
    hasEffectsWhenCalledAtPath(path) {
        if (path.length === 1) {
            const subPath = path[0];
...
const UNKNOWN_LITERAL_STRING = new class extends ValueBase {
    getReturnExpressionWhenCalledAtPath(path) {
        if (path.length === 1) {
...
class UnknownObjectExpression extends ValueBase {
    constructor() {
        super(...arguments);
        this.included = false;
    }
...
class Variable {
    ...
    mayModifyThisWhenCalledAtPath(_path, _recursionTracker) {
        return true;
    }
...
class LocalVariable extends Variable {
    ...
    mayModifyThisWhenCalledAtPath(path, recursionTracker) {
        if (this.isReassigned || !this.init || path.length > MAX_PATH_DEPTH) {
            return true;
        }
        const trackedEntities = recursionTracker.getEntities(path);
...
class FunctionNode extends NodeBase {
    ...
    mayModifyThisWhenCalledAtPath(path) {
...
class MemberExpression extends NodeBase {
    ...
    mayModifyThisWhenCalledAtPath(path, recursionTracker) {
        ...
        return this.object.mayModifyThisWhenCalledAtPath([this.propertyKey].concat(path), recursionTracker);
    }
...
class ObjectExpression extends NodeBase {
    ...
    mayModifyThisWhenCalledAtPath(path, recursionTracker) {
        ...
        const property = (_a = this.getPropertyMap()[path[0]]) === null || _a === void 0 ? void 0 : _a.exactMatchRead;
        return property
